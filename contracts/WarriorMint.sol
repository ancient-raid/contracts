...
contract WarriorMint is Ownable {
    ...
    constructor(
        ...
        paymentAddrs[IWarrior.SpendCoin.Gold] = gold_;
        paymentAddrs[IWarrior.SpendCoin.Silver] = silver_;
        paymentAddrs[IWarrior.SpendCoin.Copper] = copper_;

        prices[IWarrior.SpendCoin.Gold] = 1e18;
        prices[IWarrior.SpendCoin.Silver] = 1e18;
        prices[IWarrior.SpendCoin.Copper] = 1e18;
        busdPrice = 1 * 1e18;
        raidPrice = 1700 * 1e18;
    ...
    function setRaidPrice(uint256 price_) external {
        require(isOperator[msg.sender], "Not operator");
        raidPrice = price_;
    }
    ...
    function mintBy(IWarrior.SpendCoin spendCoin, uint256 count) external onlyEOA {
        require(spendCoin != IWarrior.SpendCoin.Busd, "Invalid payment");
        IERC20(paymentAddrs[spendCoin]).safeTransferFrom(msg.sender, recipient, count * prices[spendCoin]);
        for (uint256 i = 0; i < count; i++) {
            IWarrior(warrior).mint(msg.sender, spendCoin);
        }
    }
}
