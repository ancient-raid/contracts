...
import type { Blacklist, Blacklist__factory, Hero, Hero__factory } from "../src/types";
...
describe("Hero", function () {
  before(async function () {
    ...
    this.hero = <Hero>await heroFactory.deploy();
  ...
  describe("mint", function () {
    it("mint hero", async function () {
      await this.hero.mint(this.admin.address);
  ...
  describe("transferFrom", function () {
    it("should be reverted if spender is blocked", async function () {
      await this.hero.mint(this.admin.address); // id = 1

      await expect(this.hero.approve(this.blacklisted.address, 1)).to.be.revertedWith("Hero: Blocked operator");

      await expect(
        this.hero.connect(this.blacklisted).transferFrom(this.admin.address, this.user.address, 1),
      ).to.be.revertedWith("Hero: Blocked operator");
    });
